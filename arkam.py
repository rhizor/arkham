#!/usr/bin/env python3
"""
CTF Agent - AI-powered assistant for solving CTF challenges
Supports: HackTheBox, TryHackMe, picoCTF, and other platforms
...
# Configuración
CTF_AGENT_DIR = Path.home() / ".ctf_agent"
SESSION_FILE = CTF_AGENT_DIR / "sessions"
LOGS_DIR = CTF_AGENT_DIR / "logs"
HISTORY_FILE = CTF_AGENT_DIR / "history.json"

for d in [CTF_AGENT_DIR, SESSION_FILE, LOGS_DIR]:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOGS_DIR / "ctf_agent.log"),
...
class CTFTools:
    ...
    def nmap_scan(target: str, ports: str = "-p-", flags: str = "-sVC") -> str:
        """Escaneo con nmap."""
        cmd = f"nmap {flags} {ports} {target} -oA {CTF_AGENT_DIR}/nmap/{target.replace('.', '_')}"
        logger.info(f"Running: {cmd}")
        
        # Ensure nmap directory exists
        (CTF_AGENT_DIR / "nmap").mkdir(exist_ok=True)
...
╚══════════════════════════════════════════════════════════════════╝
        ...
        while True:
            try:
                user_input = input("ctf-agent> ").strip()
                if not user_input:
                    continue
